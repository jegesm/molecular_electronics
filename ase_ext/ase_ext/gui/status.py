...
from ase_ext.data import chemical_symbols as symbols
from ase_ext.data import atomic_names as names
from ase_ext.gui.widgets import pack
...
class Status:
    ...
    def status(self):
        ...
        if n == 1:
            ...
            text = ( 'dihedral %s->%s->%s->%s: %.1f°' % tuple([symbols[z] for z in Z]+[angle]))
        else:
            text = ' ' + formula(Z)
