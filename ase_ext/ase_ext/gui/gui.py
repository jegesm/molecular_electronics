...
import gtk
from ase_ext.gui.view import View
from ase_ext.gui.status import Status
from ase_ext.gui.widgets import pack, help, Help, oops
from ase_ext.gui.languages import translate as _

from ase_ext.gui.settings import Settings
from ase_ext.gui.surfaceslab import SetupSurfaceSlab
from ase_ext.gui.nanoparticle import SetupNanoparticle
from ase_ext.gui.nanotube import SetupNanotube
from ase_ext.gui.graphene import SetupGraphene
from ase_ext.gui.calculator import SetCalculator
from ase_ext.gui.energyforces import EnergyForces
from ase_ext.gui.minimize import Minimize
from ase_ext.gui.scaling import HomogeneousDeformation
...
class GUI(View, Status):
    ...
    def modify_atoms(self, widget, data=None):
        ...
        if data:
            if data == 'OK':
                ...
                if symbol != 'X': 
                    try:
                        a = ase_ext.Atoms([ase.Atom(symbol)])  
                    except:
    ...
    def debug(self, x):
        from ase_ext.gui.debug import Debug
        Debug(self)

    def execute(self, widget=None):
        from ase_ext.gui.execute import Execute
        Execute(self)
        
    def constraints_window(self, widget=None):
        from ase_ext.gui.constraints import Constraints
        Constraints(self)

    def dft_window(self, widget=None):
        from ase_ext.gui.dft import DFT
        DFT(self)
    ...
    def movie(self, widget=None):
        from ase_ext.gui.movie import Movie
        self.movie_window = Movie(self)
        
    def plot_graphs(self, x=None, expr=None):
        from ase_ext.gui.graphs import Graphs
        g = Graphs(self)
        if expr is not None:
            g.plot(expr=expr)
    ...
    def NEB(self, action):
        from ase_ext.gui.neb import NudgedElasticBand
        NudgedElasticBand(self.images)
        
    def bulk_modulus(self, action):
        from ase_ext.gui.bulk_modulus import BulkModulus
