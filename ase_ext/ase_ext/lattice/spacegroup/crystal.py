...
import ase
from ase_ext.atoms import string2symbols
...
def crystal(symbols=None, basis=None, spacegroup=1, setting=1, 
    ...
    >>> ase_ext.view(diamond)  # doctest: +SKIP
    ...
    if isinstance(symbols, ase_ext.Atoms):
        basis = symbols
        symbols = basis.get_chemical_symbols()
    if isinstance(basis, ase_ext.Atoms):
