...
from ase_ext.data import atomic_numbers, chemical_symbols, atomic_masses
import ase_ext.units as units
...
class Atoms(object):
    ...
    ffsymbols: forcefield symbols (C -> C_R or C_3)
    ...
    calculator: calculator object
        ...
        forces.
    forces: from files e.g.
    
    bonds: This contains the bonds between atoms determined by some kind of radius (e.g. Van der Waals)
    
    Info: additional information stored in the file
    ...
    def __init__(self, symbols=None,
                 ...
                 cell=None, pbc=None,
                 constraint=None,
                 calculator=None, forces=None, energy=None,Info=None,bonds=None, ffsymbols=None):

        atoms = None
        
        if hasattr(symbols, 'GetUnitCell'):
            from ase_ext.old import OldASEListOfAtomsWrapper
            ...
            data = list(zip(*[atom.get_data() for atom in symbols]))
...
#                energy = atoms.get_energy()
            ...
            if bonds is None and atoms.has('bonds'):
                bonds = atoms.get_bonds()
            if ffsymbols is None and atoms.has('ffsymbols'):
                ffsymbols = atoms.get_ffsymbols()
            if cell is None:
                cell = atoms.get_cell()
            if pbc is None:
                pbc = atoms.get_pbc()
        ...
        if forces is None:
            forces = np.zeros((len(self.arrays['numbers']), 3))
        self.new_array('forces', forces, float, (3,))            
    ...
    def set_calculator(self, calc=None):
        ...
        if hasattr(calc, '_SetListOfAtoms'):
            from ase_ext.old import OldASECalculatorWrapper
    ...
    def has(self, name):
        ...
        'charges', 'forces', 'bonds', 'ffsymbols'."""
    ...
    def set_bonds(self, bonds):
        """Set bonds."""
#        self.set_array('bonds', bonds, float, shape=(3,))
        self.bonds=bonds
            
    def get_bonds(self):
        """Get array of bonds."""
        return self.bonds
#        if 'bonds' in self.arrays:
#            return self.arrays['bonds'].copy()
#        else:
#            return np.zeros(len(self),type="int")
            
    def set_ffsymbols (self, ffsymbols ):
        """Set ffsymbols ."""
        self.set_array('ffsymbols ', ffsymbols , str, ())
            
    def get_ffsymbols (self):
        """Get array of ffsymbols ."""
        if 'ffsymbols ' in self.arrays:
            return self.arrays['ffsymbols '].copy()
        else:
            return np.zeros(len(self))
    ...
    def do_calc(self):
        if self.calc is None:
            raise RuntimeError('Atoms object has no calculator.')
        self.calc.update(self)
    
    def set_forces(self, newforces):
        """Set forces."""
        forces = self.arrays['forces']
        if self.constraints:
            newforces = np.asarray(newforces, float)
            for constraint in self.constraints:
                constraint.adjust_forces(forces, newforces)
                
        self.set_array('forces', newforces, shape=(3,))
    ...
    def get_forces(self, apply_constraint=True):
        ...
        forces."""
        if self.has('forces') is None:
            raise RuntimeError('Atoms object has no forces.')
        else:
            return self.arrays['forces'].copy()
    ...
    def __getitem__(self, i):
        ...
        import copy
        from ase_ext.constraints import FixConstraint
    ...
    def write(self, filename, format=None,symbols=[]):
        """Write yourself to a file."""
        from ase_ext.io import write
        write(filename, self, format, symbols)
