...
class Vasp(Calculator):
    ...
    def calculate(self, atoms):
        ...
        self.run()
        # Read output
        atoms_sorted = ase_ext.io.read('CONTCAR', format='vasp')
    ...
    def restart_load(self):
        ...
        if os.path.isfile('ase-sort.dat'):
            ...
            atoms = ase_ext.io.read('CONTCAR', format='vasp')[self.resort]
        else:
            atoms = ase_ext.io.read('CONTCAR', format='vasp')
...
#        self.old_incar_params = self.incar_params.copy()
    ...
    def write_incar(self, atoms, **kwargs):
        ...
        for key, val in list(self.float_params.items()):
            if val is not None:
                incar.write(' %s = %5.6f\n' % (key.upper(), val))
        for key, val in list(self.exp_params.items()):
            if val is not None:
                incar.write(' %s = %5.2e\n' % (key.upper(), val))
        for key, val in list(self.string_params.items()):
            if val is not None:
                incar.write(' %s = %s\n' % (key.upper(), val))
        for key, val in list(self.int_params.items()):
            if val is not None:
                incar.write(' %s = %d\n' % (key.upper(), val))
        for key, val in list(self.list_params.items()):
            if val is not None:
                ...
                incar.write('\n')
        for key, val in list(self.bool_params.items()):
            if val is not None:
                incar.write(' %s = ' % key.upper())
                if val:
                    incar.write('.TRUE.\n')
                    ...
                    incar.write('.FALSE.\n')
        for key, val in list(self.special_params.items()):
            if val is not None:
                incar.write(' %s = ' % key.upper())
                if key is 'lreal':
...
# The below functions are used to restart a calculation and are under early constructions
    ...
    def read_potcar(self):
        ...
        if xc_flag not in list(xc_dict.keys()):
            raise ValueError(
...
class xdat2traj:
    def __init__(self, trajectory=None, atoms=None, poscar=None, 
        ...
        if not poscar:
            ...
            self.poscar = poscar
        if not atoms:
            self.atoms = ase_ext.io.read(self.poscar, format='vasp')
        else:
            self.atoms = atoms
        if not xdatcar:
        ...
        self.out = ase_ext.io.trajectory.PickleTrajectory(self.trajectory, mode='w')
