...
from ase_ext.units import Hartree, Bohr
...
class Dftb:
    ...
    def __init__(self, label='dftb', write_dftb=False,
        ...
        if not(write_dftb):
            if os.path.isfile('dftb_in.hsd'):
                ...
                raise RuntimeError('Provide it or set write_dftb=True ')
            #lines = f.readlines()
            f.close()
    ...
    def calculate(self, atoms):
        ...
        if ('DFTB_COMMAND' in os.environ and
            ('DFTB_PREFIX' in os.environ)):
            dftb = os.environ['DFTB_COMMAND']
            exitcode = os.system(dftb+ '> dftb.output' )
        elif not('DFTB_COMMAND' in os.environ):
            raise RuntimeError('Please set DFTB_COMMAND environment variable')
        elif not('DFTB_PREFIX' in os.environ):
        ...
        if self.do_spin_polarized:
            ...
            for i in self.allspecies:
                if i == 'H':
                    ...
                    raise RuntimeError('Run spin unpolarised calculation')
                fh.write('   }\n') 
        else:
