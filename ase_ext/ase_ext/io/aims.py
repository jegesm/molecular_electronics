...
def read_aims_calculator(file):
    ...
    while True:
        ...
        if "=======================================================" in line:
            ...
            if key == '#':
                comment = True   
            elif key in calc.float_params:
                calc.float_params[key] = float(args[1])
            elif key in calc.exp_params:
                calc.exp_params[key] = float(args[1])
            elif key in calc.string_params:
                ...
                key = key[1:]
                if key in calc.input_parameters:
